...
class PrimitivesParser extends Transform {
    ...
    _transform(chunk, enc, cb) {
        if (chunk.type === 'OSMHeader') {
            const osmheader = parsers.osm.HeaderBlock.decode(chunk.data);
            this._HistoricaInformation = osmheader.required_features.includes('HistoricalInformation');
        } else if (chunk.type === 'OSMData') {
            // TODO: improve backpressure handling (use Duplex stream instead of Transform)
            for (const node of parseChunk(chunk, this._HistoricaInformation)) {
                this.push(node);
            }
        }
